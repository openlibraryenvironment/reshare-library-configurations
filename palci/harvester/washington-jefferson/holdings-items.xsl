    ...
    <holdingsRecords>
      ...
      <xsl:if test="marc:datafield[@tag='952']">
        <xsl:for-each select="marc:datafield[@tag='952']">
          ...
          <xsl:sort select="./marc:subfield[@code='e']"/>
          <xsl:variable name="prenode" select="./preceding-sibling::marc:datafield[@tag='952'][1]"/>
          <xsl:variable name="prekey">
            <xsl:value-of select="concat($prenode/marc:subfield[@code='d'], ' ', $prenode/marc:subfield[@code='e'])"/>
          </xsl:variable>
          <xsl:variable name="curkey">
            <xsl:value-of select="concat(./marc:subfield[@code='d'], ' ', ./marc:subfield[@code='e'])"/>
          </xsl:variable>
          <!-- <preKey><xsl:value-of select="$prekey"/></preKey> -->
          <xsl:variable name="iid" select="./marc:subfield[@code='m']"/>
          <xsl:variable name="callno" select="./marc:subfield[@code='e']"/>
          ...
          <xsl:if test="not($curkey=$prekey)">
            <i>
              ...
              <illPolicyId>
                <xsl:choose>
                  <xsl:when test="$loc='IRC'">46970b40-918e-47a4-a45d-b1677a2d3d46</xsl:when>
                  <xsl:when test="$loc='Music'">46970b40-918e-47a4-a45d-b1677a2d3d46</xsl:when>
                  <xsl:when test="$loc='Oversize'">46970b40-918e-47a4-a45d-b1677a2d3d46</xsl:when>
                  <xsl:when test="$loc='Plays'">46970b40-918e-47a4-a45d-b1677a2d3d46</xsl:when>
                  <xsl:when test="$loc='Reference'">46970b40-918e-47a4-a45d-b1677a2d3d46</xsl:when>
                  <xsl:when test="$loc='Stacks'">46970b40-918e-47a4-a45d-b1677a2d3d46</xsl:when>
                  <xsl:when test="$loc='Video'">46970b40-918e-47a4-a45d-b1677a2d3d46</xsl:when>
              ...
              <xsl:variable name="ctype" select="./marc:subfield[@code='j']"/>
              <callNumberTypeId>
                <xsl:choose>
                  <xsl:when test="$ctype='Library of Congress classification'">95467209-6d7b-468b-94df-0f5d7ad2747d</xsl:when> <!-- LC -->
                  <xsl:when test="$ctype='Dewey Decimal classification'">03dd64d0-5626-4ecd-8ece-4531e0069f35</xsl:when> <!-- Dewey -->
                  <xsl:when test="$ctype='NLM'">054d460d-d6b9-4469-9e37-7a78a2266655</xsl:when> <!-- NLM -->
              ...
              <items>
                <arr>
                <xsl:for-each select="../marc:datafield[@tag='952' and ./marc:subfield[@code='d']=$loc and ./marc:subfield[@code='e']=$callno]">
                  <i>

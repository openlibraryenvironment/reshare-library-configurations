          ...
          <arr>
            <xsl:for-each select="marc:datafield[@tag='900' and @ind1!='4' and @ind1!=' ']">
              ...
              <i>
                ...
                <illPolicyId>
                  <xsl:choose>
                    <xsl:when test="$LOC='OFFSITE BY-REQUEST'"><xsl:value-of select="$WILL_LEND"/></xsl:when>
                    <xsl:when test="$LOC='MELLON NEWBOOK'"><xsl:value-of select="$WILL_LEND"/></xsl:when>
                    <xsl:when test="$LOC='MELLON READING-RM'"><xsl:value-of select="$WILL_LEND"/></xsl:when>
                    <xsl:when test="$LOC='ENGR-SCI NEWBOOK'"><xsl:value-of select="$WILL_LEND"/></xsl:when>
                    <xsl:when test="$LOC='ENGR-SCI STACKS'"><xsl:value-of select="$WILL_LEND"/></xsl:when>
                    <xsl:when test="$LOC='HUNT CAREER-2'"><xsl:value-of select="$WILL_LEND"/></xsl:when>
                    <xsl:when test="$LOC='HUNT FOLIOS-2'"><xsl:value-of select="$WILL_LEND"/></xsl:when>
                    <xsl:when test="$LOC='HUNT FOLIOS-4'"><xsl:value-of select="$WILL_LEND"/></xsl:when>
                    <xsl:when test="$LOC='HUNT NEWBOOK-1'"><xsl:value-of select="$WILL_LEND"/></xsl:when>
                    <xsl:when test="$LOC='HUNT OVRSZQ-3'"><xsl:value-of select="$WILL_LEND"/></xsl:when>
                    <xsl:when test="$LOC='HUNT OVRSZQ-4'"><xsl:value-of select="$WILL_LEND"/></xsl:when>
                    <xsl:when test="$LOC='HUNT STACKS-2'"><xsl:value-of select="$WILL_LEND"/></xsl:when>
                    <xsl:when test="$LOC='HUNT STACKS-3'"><xsl:value-of select="$WILL_LEND"/></xsl:when>
                    <xsl:when test="$LOC='HUNT STACKS-4'"><xsl:value-of select="$WILL_LEND"/></xsl:when>
                    <xsl:when test="$LOC='HUNT FA-NEWBK-4'"><xsl:value-of select="$WILL_LEND"/></xsl:when>
                    ...
                    <xsl:for-each select="../marc:datafield[@tag='995']">
                      <xsl:if test="marc:subfield[@code='ff']=$holdingsId">
                        <i>
                          ...
                          <numberOfPieces><xsl:value-of select="./marc:subfield[@code='jj']"/></numberOfPieces>
                          <permanentLocationIdHere><xsl:value-of select="concat(marc:subfield[@code='i'], ' ', marc:subfield[@code='aa'])"/></permanentLocationIdHere>
                        </i>
                      </xsl:if>
                    </xsl:for-each>
                  </arr>

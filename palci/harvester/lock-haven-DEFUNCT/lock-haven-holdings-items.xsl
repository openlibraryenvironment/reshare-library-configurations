          ...
          <arr>
            <xsl:for-each select="marc:datafield[@tag='900' and @ind1!='4' and @ind1!=' ']">
              ...
              <i>
                ...
                <illPolicyId>
                  <xsl:choose>
                    <xsl:when test="$LOC='CALD'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                    <xsl:when test="$LOC='CDMUS'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                    <xsl:when test="$LOC='CHIL'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                    <xsl:when test="$LOC='CLEAR'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                    <xsl:when test="$LOC='CLEARADBK'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                    <xsl:when test="$LOC='CLEARCAR'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                    <xsl:when test="$LOC='CLEARCHIL'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                    <xsl:when test="$LOC='CUR'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                    <xsl:when test="$LOC='E-CHIL'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                    <xsl:when test="$LOC='FOL'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                    <xsl:when test="$LOC='LREAD'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                    <xsl:when test="$LOC='MAIN'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                    <xsl:when test="$LOC='NBC'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                    <xsl:when test="$LOC='NBS'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                    <xsl:when test="$LOC='NEWBERY'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                    <xsl:when test="$LOC='PICBOO'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                    <xsl:when test="$LOC='TFOL'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                    <xsl:when test="$LOC='PER'"><xsl:value-of select="$WILL_LEND" /></xsl:when>

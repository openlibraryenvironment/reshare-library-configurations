...
<xsl:stylesheet
  ...
  <xsl:template match="//marc:record">
    ...
    <holdingsRecords>
      <arr>
      <xsl:if test="marc:datafield[@tag='Z30']">
        <xsl:for-each select="marc:datafield[@tag='Z30']">
          <xsl:sort select="./marc:subfield[@code='2']"/>
          <xsl:variable name="preloc">
            <xsl:value-of select="./preceding-sibling::marc:datafield[@tag='Z30'][1]/marc:subfield[@code='2']"/>
          </xsl:variable>
          <xsl:variable name="loc" select="./marc:subfield[@code='2']"/>
            ...
            <i>
              ...
              <illPolicyId>
                <xsl:choose>
                  ...
                  <xsl:when test="$loc='MAINO'">46970b40-918e-47a4-a45d-b1677a2d3d46</xsl:when> <!-- Will lend -->
              ...
              <items>
                <arr>
                <xsl:for-each select="../marc:datafield[@tag='Z30']/marc:subfield[@code='2'][.=$loc]/..">
                  <i>
                    <itemIdentifier><xsl:value-of select="./marc:subfield[@code='5']"/></itemIdentifier>
                    <barcode><xsl:value-of select="./marc:subfield[@code='5']"/></barcode>
      ...
      <xsl:if test="not(marc:datafield[@tag='Z30'])">
        <i>

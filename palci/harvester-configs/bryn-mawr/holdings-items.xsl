    ...
    <holdingsRecords>
        <xsl:choose>
            <xsl:when test="marc:datafield[@tag='900' and @ind1!='4' and @ind1!=' ']">
              <arr>
                <xsl:for-each select="marc:datafield[@tag='900' and @ind1!='4' and @ind1!=' ']/marc:subfield[@code='d' and starts-with(., 'b')]/..">
                ...
                <i>
                    ...
                    <illPolicyId>
                      <xsl:choose>
                        <xsl:when test="$LOC='bc'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                        <xsl:when test="$LOC='bcf'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                        <xsl:when test="$LOC='bcff'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                        <xsl:when test="$LOC='bcsfi'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                        <xsl:when test="$LOC='bkraw'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                        <xsl:when test="$LOC='bxc'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                        <xsl:when test="$LOC='ba'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                        <xsl:when test="$LOC='baf'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                        <xsl:when test="$LOC='baff'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                        <xsl:when test="$LOC='bs'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                        <xsl:when test="$LOC='bsx'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                        <xsl:when test="$LOC='bsff'"><xsl:value-of select="$WILL_LEND" /></xsl:when>

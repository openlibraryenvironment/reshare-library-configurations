          ...
          <arr>
            <xsl:for-each select="marc:datafield[@tag='900' and @ind1!='4' and @ind1!=' ']">
              ...
              <i>
                ...
                <illPolicyId>
                  <xsl:choose>
                    <xsl:when test="$LOC='2LC'"><xsl:value-of select="$WILL_LEND"/></xsl:when>
                    <xsl:when test="$LOC='3LC'"><xsl:value-of select="$WILL_LEND"/></xsl:when>
                    <xsl:when test="$LOC='4LC'"><xsl:value-of select="$WILL_LEND"/></xsl:when>
                    <xsl:when test="$LOC='CURR'"><xsl:value-of select="$WILL_LEND"/></xsl:when>
                    <xsl:when test="$LOC='FAC'"><xsl:value-of select="$WILL_LEND"/></xsl:when>
                    <xsl:when test="$LOC='FDI'"><xsl:value-of select="$WILL_LEND"/></xsl:when>
                    <xsl:when test="$LOC='GNC'"><xsl:value-of select="$WILL_LEND"/></xsl:when>
                    ...
                    <xsl:for-each select="../marc:datafield[@tag='995']">
                      <xsl:if test="marc:subfield[@code='ff']=$holdingsId">
                        <i>
                          ...
                          <permanentLocationIdHere><xsl:value-of select="marc:subfield[@code='z']"/></permanentLocationIdHere>
                        </i>
                      </xsl:if>
                    </xsl:for-each>
                  </arr>

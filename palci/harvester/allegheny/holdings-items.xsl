          ...
          <arr>
            <xsl:for-each select="marc:datafield[@tag='900' and @ind1!='4' and @ind1!=' ']">
              ...
              <i>
                ...
                <illPolicyId>
                  <xsl:choose>
                    <xsl:when test="$LOC='bksl'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                    <xsl:when test="$LOC='bksu'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                    <xsl:when test="$LOC='dvdl'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                ...
                <xsl:for-each select="../marc:datafield[@tag='995']">
                  <xsl:if test="marc:subfield[@code='ff']=$holdingsId">
                    <callNumberTypeId>
                      <xsl:variable name="CNTYPE" select="marc:subfield[@code='u']" />
                      <xsl:choose>

          ...
          <xsl:when test="marc:datafield[@tag='952']">
            <xsl:for-each select="marc:datafield[@tag='952']">
              ...
              <xsl:variable name="preloc">
                <xsl:value-of select="concat(./preceding-sibling::marc:datafield[@tag='952'][1]/marc:subfield[@code='c'], ' ', ./preceding-sibling::marc:datafield[@tag='952'][1]/marc:subfield[@code='c'])"/>
              </xsl:variable>
              <xsl:variable name="iid" select="./marc:subfield[@code='9']"/>
              <xsl:variable name="lib" select="./marc:subfield[@code='a']"/>
              <xsl:variable name="col" select="./marc:subfield[@code='c']"/>
              <xsl:variable name="loc" select="concat($lib, ' ', $col)"/>
              <xsl:if test="not($loc=$preloc)">
                <i>
                  <permanentLocationIdHere><xsl:value-of select="$loc"/></permanentLocationIdHere>
                  <illPolicyId>
                    <xsl:choose>
                      <xsl:when test="$loc='HAMMER STACKS'">46970b40-918e-47a4-a45d-b1677a2d3d46</xsl:when> <!-- Will lend -->
                      <xsl:when test="$loc='HAMMER CURR'">46970b40-918e-47a4-a45d-b1677a2d3d46</xsl:when> <!-- Will lend -->
                      <xsl:when test="$loc='HAMMER THESIS SPEC'">46970b40-918e-47a4-a45d-b1677a2d3d46</xsl:when> <!-- Will lend -->
                      <xsl:when test="$loc='HAMMER NEWBOOK'">46970b40-918e-47a4-a45d-b1677a2d3d46</xsl:when> <!-- Will lend -->

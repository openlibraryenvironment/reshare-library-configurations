    ...
    <holdingsRecords>
      ...
      <xsl:if test="marc:datafield[@tag='930']">
        <xsl:for-each select="marc:datafield[@tag='930']">
          <xsl:sort select="./marc:subfield[@code='s']"/>
          <xsl:variable name="preloc">
            <xsl:value-of select="./preceding-sibling::marc:datafield[@tag='930'][1]/marc:subfield[@code='s']"/>
          </xsl:variable>
          <xsl:variable name="iid" select="./marc:subfield[@code='h']"/>
          <xsl:variable name="loc" select="./marc:subfield[@code='s']"/>
          <xsl:variable name="mat" select="./marc:subfield[@code='m']"/>
          <xsl:if test="not($loc=$preloc)">
            <i>
              ...
              <illPolicyId>
                <xsl:choose>
                  <xsl:when test="$loc-clean='TNSFOEZB' and ($mat='BOOK' or $mat='SCORE')">46970b40-918e-47a4-a45d-b1677a2d3d46</xsl:when> <!-- Will lend -->
                  <xsl:when test="$loc-clean='TNSGIEZB' and ($mat='BOOK' or $mat='SCORE')">46970b40-918e-47a4-a45d-b1677a2d3d46</xsl:when> <!-- Will lend -->
                  <xsl:when test="$loc-clean='TNSSCEZB' and $mat='BOOK'">46970b40-918e-47a4-a45d-b1677a2d3d46</xsl:when> <!-- Will lend -->
                  <xsl:when test="$loc-clean='TNSOSEZB' and $mat='BOOK'">46970b40-918e-47a4-a45d-b1677a2d3d46</xsl:when> <!-- Will lend -->
              ...
              <xsl:variable name="cntype" select="./marc:subfield[@code='t']"/>
              <callNumberTypeId>
                <xsl:choose>
                  <xsl:when test="$cntype='0'">95467209-6d7b-468b-94df-0f5d7ad2747d</xsl:when> <!-- LC -->
              ...
              <items>
                <arr>
                <xsl:for-each select="../marc:datafield[@tag='930']/marc:subfield[@code='s'][.=$loc]/..">
                  <i>
                    <itemIdentifier><xsl:value-of select="$iid"/></itemIdentifier>
                    <barcode><xsl:value-of select="./marc:subfield[@code='b']"/></barcode>
                    <volume><xsl:value-of select="./marc:subfield[@code='d']"/></volume>
      ...
      <xsl:if test="not(marc:datafield[@tag='930'])">
        <i>

          ...
          <arr>
            <xsl:for-each select="marc:datafield[@tag='900' and @ind1!='4' and @ind1!=' ']">
              ...
              <i>
                ...
                <illPolicyId>
                  <xsl:choose>
                    <xsl:when test="$LOC='bests'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                    <xsl:when test="$LOC='vkcc'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                    <xsl:when test="$LOC='currc'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                    <xsl:when test="$LOC='graph'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                    <xsl:when test="$LOC='paper'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                    <xsl:when test="$LOC='new'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                    <xsl:when test="$LOC='video'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                    <xsl:when test="$LOC='cd'"><xsl:value-of select="$WILL_LEND" /></xsl:when>

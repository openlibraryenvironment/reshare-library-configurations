...
<xsl:stylesheet
  ...
  <xsl:template match="//marc:record">
    <localIdentifier>fdaf</localIdentifier>
    ...
    <holdingsRecords>
      ...
      <xsl:if test="marc:datafield[@tag='949']">
        <xsl:for-each select="marc:datafield[@tag='949']">
          ...
          <xsl:variable name="iid" select="./marc:subfield[@code='g']"/>
          <xsl:variable name="loc" select="./marc:subfield[@code='a']"/>
          <xsl:if test="not($loc=$preloc)">
            <i>
              ...
              <illPolicyId>
                <xsl:choose>
                  <xsl:when test="$loc-clean='WFPBOOK'">46970b40-918e-47a4-a45d-b1677a2d3d46</xsl:when> <!-- Will lend -->
                  <xsl:when test="$loc-clean='WMHBOOK'">46970b40-918e-47a4-a45d-b1677a2d3d46</xsl:when> <!-- Will lend -->
                  <xsl:when test="$loc-clean='WSTACKBOOK'">46970b40-918e-47a4-a45d-b1677a2d3d46</xsl:when> <!-- Will lend -->
              ...
              <items>
                <arr>
                <xsl:for-each select="../marc:datafield[@tag='949']/marc:subfield[@code='a'][.=$loc]/..">
                  <i>
                    <itemIdentifier><xsl:value-of select="./marc:subfield[@code='g']"/></itemIdentifier>
                    <barcode><xsl:value-of select="./marc:subfield[@code='g']"/></barcode>
                    <volume><xsl:value-of select="./marc:subfield[@code='i']"/></volume>
                    ...
                    <xsl:if test='./marc:subfield[@code="h"]'>
                      <yearCaption>
                        <arr>
                          <i><xsl:value-of select='./marc:subfield[@code="h"]'/></i>
                        </arr>
                      </yearCaption>
                    </xsl:if>
                  </i>

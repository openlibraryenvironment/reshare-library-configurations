...
<xsl:stylesheet
  ...
  <xsl:template match="//marc:record">
    <holdingsRecords>
      <arr>
        <xsl:choose>
          <xsl:when test="marc:datafield[@tag='999']">
            <xsl:for-each select="marc:datafield[@tag='999']">
              ...
              <xsl:variable name="loc-clean" select="./marc:subfield[@code='l']"/>
              <xsl:if test="not($loc-clean=$preloc)">
                <i>
                  ...
                  <callNumberTypeId>
                    <xsl:choose>
                      <xsl:when test="./marc:subfield[@code='w']='DEWEY'">03dd64d0-5626-4ecd-8ece-4531e0069f35</xsl:when> <!-- Dewey -->
                      <xsl:when test="./marc:subfield[@code='w']='LC'">95467209-6d7b-468b-94df-0f5d7ad2747d</xsl:when> <!-- LC -->
                  ...
                  <items>
                    <arr>
                    <xsl:for-each select="../marc:datafield[@tag='999']/marc:subfield[@code='l'][.=$loc-clean]/..">
                      <i>

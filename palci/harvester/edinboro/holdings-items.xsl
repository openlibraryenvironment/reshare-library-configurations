          ...
          <arr>
            <xsl:for-each select="marc:datafield[@tag='900' and @ind1!='4' and @ind1!=' ']">
              ...
              <i>
                ...
                <illPolicyId>
                  <xsl:choose>
                    <xsl:when test="$LOC='Book4'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                    <xsl:when test="$LOC='Book5'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                    <xsl:when test="$LOC='Book6'"><xsl:value-of select="$WILL_LEND" /></xsl:when>
                ...
                <xsl:for-each select="../marc:datafield[@tag='995']">
                  <xsl:if test="marc:subfield[@code='z']=$LOC">
                    <callNumberTypeId>
